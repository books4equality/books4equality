...
router.post('/reserveBookConfirmed', function(req, res) {
	if(req.session.user) {
		...
		var criteria = {
			'_meta.barcode': req.body.barcode,
			'_meta.available': true
		}
		var set = {
			$set: {
				'_meta.available': false,
				'_meta.reservedBy': userInfo
			},
			$push: {
				'_meta.reservedDates': userInfo.reservedDate
			}
		}

		books.updateBook(criteria, set, function(err) {
			if(err) {
...
router.get('/getUsersBooks', function(req, res) {
	var criteria = {
		'_meta.reservedBy.email': req.session.user.email
	}
...
router.post('/unreserveBook', function(req, res) {
	books.findReservedBookByBarcode(req.body.barcode, function(err, book) {
		...
		books.unreserveBook(req.body.barcode, req.session.user, book, function(err, auth) {
			if(err) {
				return res.status(500).send()
			}
...
router.post('/signOutBook', function(req, res) {
	books.findReservedBookByBarcode(req.body.barcode, function(err) {
		if(err) return res.status(500).send()
		books.signOutBook(req.body.barcode, req.session.user, function(err, auth) {
			if(err) return res.status(500).send()
